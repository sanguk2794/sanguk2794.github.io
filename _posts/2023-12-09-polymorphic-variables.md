...
이와 달리, 변수는 참조변수의 타입에 따라 달라진다.

- 메서드 : 참조변수의 타입과 관계없이 항상 실제 인스턴스의 메서드가 호출된다.
- 멤버변수 : 참조변수의 타입에 따라 호출되는 멤버변수가 달라진다.
...
- 자손타입 → 조상타입 : 업캐스팅, 형변환 생략이 가능하다.
- 조상타입 → 자손타입 : 다운캐스팅, 형변환 생략이 불가능하다.
...
---
## 6. instanceof
조상 타입의 참조변수로 다루어지는 자손 타입의 객체를 자손 타입의 참조변수로 다룰 수 있도록 다운캐스팅을 해야 할 때, `instanceof` 연산자를 사용한다.
...
### 1. instanceof는 안티 패턴?
`instanceof`를 사용하면 간편하게 클래스의 타입을 확인할 수 있음에도 불구하고 `instanceof`가 아닌 다형성을 이용할 것이 권장된다.

#### 1. 캡슐화
`instanceof`를 사용하는 경우, 객체의 실제 타입을 외부에서 확인할 수 있게 된다. 이는 객체의 내부 구현을 외부로 노출시키기 때문에, 캡슐화 원칙에 위배된다.
캡슐화를 통해 각 객체가 가진 책임과 역할을 분리하는 것으로 유지보수와 확장에 있어 편리함을 얻을 수 있으나, 캡슐화가 보장되지 않기에 그 의미가 사라진다.
...
지금은 `Text.postOn()` 메서드 한 곳에서 사용되고 있지만, 수십 개의 사용처가 있다면 그 수십 곳의 사용처를 모두 수정해야 할 것이다.
...
이것은 `instanceof`를 사용하는 `postOn()` 메서드가 FaceBook, Twitter, Line의 포스트 구현을 모두 알고 있어야 하는 책임이 부가되는 것이다. 

이로 인해 각 타입에게 책임을 부여하면 되는 일이 하나의 메서드에게 모든 책임이 가중되고 있으며, 한 클래스는 하나의 책임만 가져야 한다는 객체지향프로그래밍의 원칙 중 하나인 단일 책임 원칙을 위배하게 된다.
