...
- 문자열과 비교해 IDE의 적극적인 지원을 받을 수 있다. : 자동완성, 오타검증, 텍스트 리팩토링
- 허용되는 값을 열거형 값으로 제한된다.
- 리팩토링시 변경 범위를 최소화할 수 있다. : 내용의 추가가 필요하더라도 Enum 코드 외에 수정할 필요가 없다.
- 연관 관계를 표현할 수 있다.
...
처음 선언된 `C` 열거형 값의 `ordinal`은 0이 되며, 뒤의 값들은 1씩 순서대로 더해져 각각 1, 2, 3의 값을 가진다.
하지만, 코드의 확장이 필요해져 위의 클래스의 맨 앞에 `Groovy` 열거형 값이 추가했다.
...
`Groovy` 열거형 값이 추가됨으로 인해 `C`의 `ordinal`값은 1이 증가해 1이 되었다. 값 변경의 위험이 존재하는 것이다.
...
이 메서드는 `Enum` 클래스 내부에 존재하지 않는다. 하지만, 컴파일러는 컴파일 타임에 자동으로 해당 메서드를 추가한다. 
...
> Mod```ier 'public' not allowed here
