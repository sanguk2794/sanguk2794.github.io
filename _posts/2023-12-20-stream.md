...
자바에서 지연 처리를 사용하는 이유는 효율성(efficiency)를 달성할 수 있기 때문이다. 중간 연산이 호출될 때마다 스트림의 연산을 수행한다고 가정하자.
그러면 중간 연산 메서드가 호출될 때마다 대상이 되지 않는 데이터를 포함하는 연산 결과를 생성해서 반환할 것이다. 
이로 인해 실행 시간이 길어지게 된다.
