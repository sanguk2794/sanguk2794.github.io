...
## 1. 에러 (Error)
프로그램 실행 중 어떤 원인에 의해서 오작동을 하거나 비정상적으로 종료되는 경우가 있다. 이러한 결과를 초래하는 원인을 프로그렘 에러 혹은 오류라고 한다.

### 1. 발생 시점에 따른 에러 구분
자바에서는 에러를 발생 시점에 따라 컴파일 에러와 런타임 에러의 두 가지로 구분한다.

이와 별개로, 컴파일 에러나 런타임 에러는 발생하지 않으나 개발자의 의도와 다르게 동작할 경우를 논리적 에러라고 한다.

### 2. 예외 처리 가능 여부에 따른 에러 구분
...
---
## 4. try-catch
발생한 예외를 처리해 주지 않으면 프로그램은 비정상적으로 종료되며, 처리되지 못한 예외는 JVM의 예외 처리기(UncaughtExceptionHandler)가 예외의 원인을 화면에 출력한다.
프로그램이 멈추지 않도록 예외를 제어하기 위해 자바에서는 `try-catch` 구문을 이용한다. 
...
해당 예제는 0부터 1사이의 값을 랜덤으로 호출하여 0.5 이상일 경우 `RuntimeException`을 발생시킨다.
에러가 `try` 블럭에서 발생할 경우 예외의 메세지와 스택 트레이스를 출력한 후 메서드를 종료한다.
...
만약 일치하는 `catch` 블럭을 찾게 되면 그 `catch` 블럭 내의 문장들을 수행하고 `try-catch` 구문을 빠져나가서 다음 문장들을 계속해서 수행한다.
...
만약 `catch` 블럭의 참조변수로 `Exception` 클래스를 선언할 경우 모든 예외를 처리할 수 있다.
...
필요에 따라 알맞은 예외 클래스를 상속함으로써, 사용자 정의 예외 클래스를 정의할 수 있다.
...
- 자바 표준 예외 중 쓸만한 게 없을 경우에만 만드는 것이 좋다. 
...
메서드의 선언부에 예외를 선언함으로써 메서드를 사용하려는 사람이 메서드의 선언부를 보았을 때 이 메서드를 사용하기 위해서는 어떤 예외들을 예외 처리해야 할 지 쉽게 파악할 수 있다.
...
심지어 하나의 예외에 대해서도 예외가 발생한 메서드와 호출한 메서드 양 쪽에서 처리하도록 할 수 있다.
...
`try-catch` 구문의 변형이다. 입출력과 관련된 클래스를 사용할 때 자동으로 자원을 반환할 수 있다는 장점이 있다.
...
한 예외가 다른 예외를 발생시킬 수도 있다. 이렇게 다른 예외의 원인이 된 예외를 원인 예외(Cause Exception)라고 한다.
